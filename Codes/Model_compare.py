...
years = [i for i in range(1,21)]

df=pd.DataFrame({'Consumption': elec_cons, 'COVID': '','Year':years})

model_base = ols("Consumption ~ Year", data=df).fit()

df['COVID'] = df['Year'].apply(lambda x: 1 if x in [19,20] else 0)

model_covid = ols("Consumption ~ Year + COVID", data=df).fit()

anova_results = sm.stats.anova_lm(model_base, model_covid,typ=1)
